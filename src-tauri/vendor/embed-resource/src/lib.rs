...
#[cfg(not(target_os = "windows"))]
mod non_windows;
// Patched for Grok Build: force the MinGW windres path even on the MSVC
// target so we can build without a full Visual Studio install (rc.exe).
...
// Patched for Grok Build: the windres backend must also be available on
// MSVC targets (we substitute MinGW windres for rc.exe).
mod windres {
