...
impl AcpClient {
    ...
    /// The `session/prompt` RPC ids still awaiting their result (oldest first).
    pub(crate) fn pending_prompt_ids(&self) -> Vec<u64> {
    ...
    fn schedule_prompt_complete_fallback(self: &Arc<Self>, stop_reason: String) {
        // Pin the RPC this notification belongs to (the oldest pending prompt,
        // or none). Completing *all* pending prompts here raced with a queued
        // follow-up: prompt A's early complete fired while follow-up B was
        // already pending on the wire, and the fallback resolved B's RPC too —
        // the Host then dropped B's stream chunks as "replay" and the reply
        // only surfaced after a session reload.
        let pinned_ids = self.pending_prompt_ids();
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let still_pending = {
                    ...
                    pinned_ids.iter().any(|id| pending.contains_key(id))
                };
                if !still_pending {
                    return;
                }
            ...
            this.complete_pending_prompt_fallback(&stop_reason, &pinned_ids);
        });
    }

    /// If agent never returned a `session/prompt` result after `prompt_complete`,
    /// free **only** the pinned RPCs — never a follow-up prompt queued behind it.
    fn complete_pending_prompt_fallback(&self, stop_reason: &str, pinned_ids: &[u64]) {
...
impl AcpClient {
    ...
    /// Send `session/prompt` with text + image content blocks (ACP-native).
    ///
    /// Images travel as base64 `{"type":"image","data","mimeType"}` blocks so
    /// the CLI's `parse_prompt_with_skills` extracts real pixels (its own
    /// `@path` / `[Image #N: path]` parsing is allowlisted to cwd/home and is
    /// not fed by the App's `@path` refs — previously images were only sent as
    /// text refs and multimodal models never saw the pixels).
    pub async fn prompt_with_images(
        ...
        for img in images {
            blocks.push(ContentBlockData::Image {
                data: img.data.clone(),
    ...
    async fn prompt_with_blocks(&self, blocks: &[ContentBlockData]) -> Result<String, AgentError> {
        ...
        let prompt: Vec<Value> = blocks.iter().map(|b| b.to_value()).collect();
...
/// One image payload for `session/prompt` (base64 data + MIME).
#[derive(Debug, Clone)]
pub struct PromptImageData {
    pub data: String,
    pub mime_type: String,
}

/// A single `session/prompt` content block (text or base64 image).
#[derive(Debug, Clone)]
...
impl ContentBlockData {
    fn to_value(&self) -> Value {
        match self {
            ContentBlockData::Text(text) => json!({ "type": "text", "text": text }),
