...
/// Whether the active main model is text-only (no image pixels).
///
/// `current` is the composer-selected model id of the live session — may be a
/// provider section id (`puls`) or an upstream model id (`gpt-5.6-luna`);
/// `None` falls back to `[models].default`. This makes Host vision follow the
/// model actually used in the conversation, not the global default.
pub fn main_is_text_only_for(
    current: Option<&str>,
    text: &str,
    list: &crate::providers::ProvidersListResult,
) -> bool {
    ...
    let picked = current
        .map(str::trim)
        .filter(|s| !s.is_empty() && *s != crate::providers::OFFICIAL_DEFAULT_MODEL);
    let def = match picked {
        Some(c) => c.to_string(),
        None => main_default_from_text(text),
    };
    if def.is_empty() || def == crate::providers::OFFICIAL_DEFAULT_MODEL {
        return list.active_source != "official";
    }
    // Explicit [model.<id>].supports_vision wins over name heuristics: custom
    // vision channels (puls / paidyun, gpt-5.x upstream ids) would otherwise
    // fall into the "unknown custom → text-only" safety branch below and the
    // Host would strip image @path refs instead of letting the CLI send pixels.
    if let Some(sv) = get_table_string(text, &format!("model.{def}"), "supports_vision") {
...
/// Back-compat entry: judge by `[models].default` (no live-session context).
pub fn main_is_text_only(text: &str, list: &crate::providers::ProvidersListResult) -> bool {
    main_is_text_only_for(None, text, list)
}
...
/// Cap on images attached as ACP content blocks per prompt (mirrors
/// [`MAX_VISION_IMAGES`]; the CLI re-encodes under its own 1.5 MB cap).
const MAX_PROMPT_IMAGE_BYTES: u64 = 6 * 1024 * 1024;

/// Split `@path` image refs out of a prompt into base64 ACP content blocks.
///
/// For multimodal main models the prompt keeps `@/abs/img.png` refs (Host
/// vision only strips for text-only mains). The CLI, however, only extracts
/// pixels from ACP `Image` content blocks — it treats a bare `@path` as a text
/// file reference, so multimodal models saw the path string but never the
/// image. This loads the files and returns them as wire-ready blocks while
/// removing the refs from the text.
///
/// Returns `(text_without_image_lines, images)`. No-op (empty images) when the
/// prompt has no `@path` image refs (plain text / Host-description path).
pub fn split_prompt_images(prompt: &str) -> (String, Vec<crate::acp_client::PromptImageData>) {
    use base64::Engine;
    let images = extract_image_at_paths(prompt);
    if images.is_empty() {
        return (prompt.to_string(), Vec::new());
    }
    let (body, _) = strip_image_at_paths(prompt);
    let mut out = Vec::with_capacity(images.len());
    for path in images.iter().take(MAX_VISION_IMAGES) {
        match std::fs::metadata(path) {
            Ok(meta) if meta.len() <= MAX_PROMPT_IMAGE_BYTES => {}
            Ok(_) => {
                tracing::warn!(target: "models_aux", path, "prompt image exceeds byte cap — skipped");
                continue;
            }
            Err(e) => {
                tracing::warn!(target: "models_aux", path, error = %e, "prompt image unreadable — skipped");
                continue;
            }
        }
        match std::fs::read(path) {
            Ok(bytes) => {
                ...
                tracing::info!(
                    ...
                    path,
                    bytes = bytes.len(),
                    "attaching image as ACP content block"
                );
                out.push(crate::acp_client::PromptImageData { data, mime_type });
            }
            Err(e) => {
                tracing::warn!(target: "models_aux", path, error = %e, "prompt image read failed — skipped");
            }
        }
    }
    (body, out)
...
///
/// `current` is the live-session model id (see [`main_is_text_only_for`]).
/// Official Grok subscription route: always false (native multimodal).
pub fn host_vision_will_run(prompt: &str, current: Option<&str>) -> bool {
    ...
    if !main_is_text_only_for(current, &config, &list) {
        return false;
    }
...
pub async fn prepare_agent_prompt_for_main(prompt: &str) -> String {
    prepare_agent_prompt_for_main_detailed(prompt, None, None)
        .await
        .prompt
}
...
"#;
    ...
    #[test]
    fn supports_vision_flag_overrides_heuristics() {
        let mut text = r#"
[models]
default = "puls"
...
[model.puls]
model = "gpt-5.6-terra"
base_url = "https://pulseaify.com/v1"
api_key = "sk-x"
...
"#
        ...
        let list = crate::providers::ProvidersListResult {
            ...
            default_model: Some("puls".into()),
            active_source: "custom".into(),
            active_provider_id: Some("puls".into()),
            config_path: String::new(),
            agent_home: String::new(),
        };
        assert!(
            !main_is_text_only(&text, &list),
            "supports_vision=true must mark the main model as vision-capable"
        );
        text = text.replace("supports_vision = true", "supports_vision = false");
        assert!(
            main_is_text_only(&text, &list),
            "supports_vision=false must mark the main model as text-only"
        );
    }

    #[test]
    fn no_vision_flag_falls_back_to_heuristics() {
        // Unknown custom id without a flag stays text-only (old safety default).
        let text = r#"
[models]
default = "puls"
...
"#;
        ...
        assert!(main_is_text_only(&text, &list));
    }
}
